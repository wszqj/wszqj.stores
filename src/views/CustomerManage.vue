...
import {
  Edit,
  Delete, Star, Search, RefreshRight, Plus
} from '@element-plus/icons-vue'
import {ref} from 'vue'
import {
  customersAddService,
  customersPageQueryService,
  customerUpdateService, deleteCustomerService,
...
const grade = ref('')
//定义函数检查表单的数据是否输入完整
const checkCustomerDate = (rule, value, callback) => {
  if (value === '') {
...
const rules = {
  name: [
    {required: true, message: '请输入客户对象姓名', trigger: 'blur'},
    {min: 2, max: 10, message: '长度为2~10位的非空字符', trigger: 'blur'},
    {validator: checkCustomerDate, trigger: 'blur'}
  ],
  phone: [
  ...
  address: [
    ...
    {min: 2, max: 50, message: '请按要求输入', trigger: 'blur'},
  ],
  grade: [
    {required: true, message: '请输入客户星级', trigger: 'blur'},
    {validator: checkGrade, trigger: 'blur'}
  ],
  integrity: [
    {required: true, message: '请输入客户积分', trigger: 'blur'},
    {min: 0, message: '客户积分必须大于等于0', trigger: 'blur'},
  ]
}
...
const customersList = async () => {
  ...
  // 调用接口获取数据
  let result = await customersPageQueryService(params);
  // 渲染视图
  total.value = result.data.total;
  // 使用Vue提供的响应式API或者ref的value属性进行修改
  customers.value = result.data.records;
}
customersList();
...
const customerAdd = async () => {
  // 判断用户输入的信息是否合法(是否为空)
  const isFormValid = Object.values(customersModel.value).some(value => String(value).trim() !== '');
  if (!isFormValid) {
    ElMessage.error("客户信息不能为空");
    return; // 如果所有字段都为空或只包含空格，则显示错误信息并停止执行
  }
  //调用接口添加数据
  let result = await customersAddService(customersModel.value);
  ElMessage.success(result.message ? result.message : '添加客户')
  // 设置抽屉状态
...
// 获取用户详细信息
const getCustomerInfo=async (row) => {
  //设置抽屉的标题
  title.value = '编辑客户'
  //设置抽屉状态
  visibleDrawer.value = true;
  //获取客户详细信息
  let result = await getCustomerInfoService(row.id);
  //渲染数据
  customersModel.value = result.data
}
...
const customerUpdate = async () => {
  ...
  //调用接口更新客户信息
  let result = await customerUpdateService(customersModel.value);
...
const deleteCustomer = (row) => {
  ElMessageBox.confirm(
      ...
      .then(async () => {
        //调用接口
        let result = await deleteCustomerService(row.id);
...
<!--        <span class="card">客户管理</span>-->
        <div class="extra">
          <el-button @click="clearCustomersModel(); title='添加客户'; visibleDrawer=true" id="add">
    ...
    <el-form inline class="selectFrom">
      <el-form-item label="客户姓名">
        <el-input v-model="name" placeholder="客户姓名" clearable/>
      </el-form-item>
      <el-form-item label="客户地址">
        <el-input v-model="address" placeholder="客户地址" clearable/>
      </el-form-item>
      <el-form-item label="客户星级">
        <el-input v-model="grade" placeholder="客户星级" clearable/>
      </el-form-item>
      <el-form-item style="float: right">
        <el-button @click="customersList()">
          <el-icon><Search /></el-icon>
          <span>搜索</span>
        </el-button>
        <el-button @click="name='';address='';grade=''; customersList()">
    ...
    <!--数据展示区-->
    <el-table :data="customers" style="width: 1430px">
...
<!--      单独修改该列的颜色-->
      <el-table-column prop="integration" label="积分">
        <template #default="{ row }">
          <span style="color: #ff605c">{{ row.integration }}</span>
        </template>
      </el-table-column>
...
<!--          星星图标-->
      ...
      <el-table-column fixed="right" label="操作" width="120">
        <template #default="{row}" >
          <el-button :icon="Edit"  class="updateButton" plain type="primary" @click="getCustomerInfo(row)" circle/>
          <el-button :icon="Delete" class="deleteButton" plain type="danger" @click="deleteCustomer(row)" circle/>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-drawer v-model="visibleDrawer" :title="title" direction="rtl" size="50%">
      ...
      <!-- 添加文章表单 -->
      <el-form :model="customersModel" label-width="100px" :rules="rules" ref="form">

        <el-form-item label="客户姓名" prop="name">
          <el-input v-model="customersModel.name" placeholder="请输入客户姓名"></el-input>
        </el-form-item>
        <el-form-item label="联系方式" prop="phone">
          <el-input v-model="customersModel.phone" placeholder="联系方式"></el-input>
        </el-form-item>
        <el-form-item label="地址" prop="address">
          <el-input v-model="customersModel.address" placeholder="地址"></el-input>
        </el-form-item>
        <el-form-item label="星级" prop="grade">
          <el-input v-model="customersModel.grade" placeholder="星级"></el-input>
        </el-form-item>
        <el-form-item label="积分" prop="integration">
          <el-input v-model="customersModel.integration" placeholder="积分"></el-input>
        </el-form-item>
        <el-form-item label="创建人">
          <el-input v-model="customersModel.createUser" placeholder="创建人" disabled></el-input>
        </el-form-item>
        <el-form-item label="修改人">
          <el-input v-model="customersModel.updateUser" placeholder="修改人" disabled></el-input>
        </el-form-item>
        <el-form-item label="创建时间">
          <el-input v-model="customersModel.createTime" placeholder="创建时间" disabled></el-input>
        </el-form-item>
        <el-form-item label="修改时间">
          <el-input v-model="customersModel.updateTime" placeholder="修改时间" disabled></el-input>
        </el-form-item>
        <!--        提交按钮-->
        <el-form-item>
          <el-button @click="title==='添加客户'?customerAdd(): customerUpdate()" id="accomplish">完成</el-button>
